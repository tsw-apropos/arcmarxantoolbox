...
As of 2025-12-19, this project will no longer be actively maintained by the author; if you wish to update it to work with ArcGIS Pro or create a new version for that purpose, please feel free to fork the project and do so.
...
3. Open ArcMap
4. Open ArcToolbox
5. Right-click at top and choose Add Toolbox...
6. From the look in drop down list select the location of your file. If you used the folders suggested above it will be in Toolboxes and then System Toolboxes or My Toolboxes
7. Select ArcMarxan.pyt and click open
8. Open ArcToolbox again if it closed
9. Right-click at top and choose Save Settings and To Default

## Use Overview

The ArcMarxan Toolbox has the following work-flow design:
...
The arcmarxan Toolbox consists of nine tools in four groups. The groups and tools are organized as follows:
...
This tool provides the means to adjust SPF values to ensure that all targets are met. Note that if you are using version 4.0.5 of Marxan, you need to set the VERBOSITY value to 4 or higher in the input.dat file so that the MarOptTotalAreas.csv file gets created. 
 ...
 * Marxan Feature Name - Each feature is assigned a name in the spec.dat file. When using the arcmarxan Toolbox this name is the field name in the source planning unit layer.
