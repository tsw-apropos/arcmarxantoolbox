...
class Toolbox(object):
    def __init__(self):
        ...
        .pyt file)."""
        self.label = "ArcMarxan Toolbox (2.0.4 for ArcPro)"
...
class MarxanProject(object):
    ...
    def createInputFile(self,outFName):
        ...
        f = open(outFName, 'w', newline='')
        creditText = "Input file for Annealing program.\n\n"
        creditText += "This file generated by ArcMarxan Toolbox version 2.0.4\n" 
...
class Boundary(object):
    ...
    def execute(self, parameters, messages):
        ...
        # ArcGIS Pro may not have arcpy.env.workspace set for Python toolboxes.
        scratch_workspace = arcpy.env.scratchGDB or arcpy.env.scratchFolder or arcpy.env.workspace
        if not scratch_workspace:
            scratch_workspace = marxan_input_folder
        arcpy.env.overwriteOutput = True

        # buffer and dissolve layer in one step
        buffFeatClass = arcpy.CreateUniqueName('amt_temp_buffer', scratch_workspace)
        ...
        unionFeatClass = arcpy.CreateUniqueName('amt_temp_union', scratch_workspace)
        ...
        oFile = open(oFileName,'w',newline='')
...
class Features(object):
    ...
    def getParameterInfo(self):
        ...
        # initial targets
        target_type = arcpy.Parameter(
            displayName="Select type of initial targets",
            name="target_type",
            datatype="GPString",
            parameterType="Required",
            direction="Input")
        target_type.filter.list = ["Proportion","Target Value","Target Occurrences"]
        target_type.value = "Proportion"

        # target value
        target_value = arcpy.Parameter(
            displayName="Set the initial target value",
            name="target_value",
            datatype="GPDouble",
            parameterType="Required",
            direction="Input")
        ...
        # spf value
        spf_value = arcpy.Parameter(
            displayName="Set the initial SPF value",
            name="spf_value",
            datatype="GPDouble",
            parameterType="Required",
            direction="Input")
        ...
        params = [pu_layer, pu_field, feature_field_names, target_type, target_value, spf_value, marxan_input_folder]
        return params
    ...
    def execute(self, parameters, messages):
        ...
        target_type = parameters[3].valueAsText
        target_value = parameters[4].value 
        spf_value = parameters[5].value
        marxan_input_folder = parameters[6].valueAsText
        # create field list
        fldsRef = []
        flds =[]
        ...
        self.createFeatFile(fldsRef,specFName,target_type,target_value,spf_value)
    ...
    def createFeatFile(self,fieldRefList,specFName,target_type,target_value,spf_value):
        ...
        f = open(specFName,'w',newline='')
        f.write(header)
        if target_type == "Proportion":
            for rec in fieldRefList:
                f.write('%d\t%.4f\t0.0\t0\t%.4f\t%s\n' % (rec[0],target_value,spf_value,rec[1]))
        elif target_type == "Target Value":
            for rec in fieldRefList:
                f.write('%d\t0.0\t%.4f\t0\t%.4f\t%s\n' % (rec[0],target_value,spf_value,rec[1]))
        else: # "Target Occurences"
            for rec in fieldRefList:
                f.write('%d\t0.0\t0.0\t%d\t%.4f\t%s\n' % (rec[0],round(target_value),spf_value,rec[1]))
        f.close()
...
class SelectedSummary(object):
    ...
    def execute(self, parameters, messages):
        ...
        if os.path.exists(specFile):
            ...
            specRecs = {}
            with open(specFile,'r',newline='') as csvfile:
        ...
        if os.path.exists(puvspFile):
            ...
            featSummary = {}
            f = open(puvspFile,'r',newline='')
            sniffer = csv.Sniffer()
            contents = f.readline()
            dialect = sniffer.sniff(contents)
            f.close()           
            with open(puvspFile,'r',newline='') as csvfile:
...
class EstimateBLM(object):
    ...
    def execute(self, parameters, messages):
        ...
        if os.path.exists(inFile):
            ...
            raise arcpy.ExecuteError
        if reps is None or inDir is None or outDir is None or scenName is None:
            arcpy.AddError('input.dat is missing one or more required values: NUMREPS, INPUTDIR, OUTPUTDIR, SCENNAME')
            raise arcpy.ExecuteError
        #
        ...
        # 1b. do run
        status, messageText = runMarxanOnce(marxanExe,projDir,outDir,scenName,reps)
        self.raiseIfMarxanError(status,messageText)
        ...
        try:
            ...
            calcSlope = self.calculateSlope(xCost,yCost,xBoundary,yBoundary,'X','Y')
            if calcSlope is None:
                arcpy.AddMessage('Cannot estimate BLM: X and Y boundary lengths are the same or too close to compare.')
                return
            # 3b. Set blm to "slope"
        ...
        bVal = self.calculateSlope(zCost,yCost,zBoundary,yBoundary,'Z','Y')
        cVal = self.calculateSlope(xCost,zCost,xBoundary,zBoundary,'X','Z')
        if bVal is None or cVal is None:
            ...
            f.close()
            arcpy.AddMessage('Cannot estimate BLM range: point Z duplicates the boundary length of X or Y.')
            return
        minRange = min(bVal,cVal)
        maxRange = max(bVal,cVal)
        #
        f = open(fName,'w', newline='')
        f.write('BLM Estimation Results\n\n')
        f.write('Minimum Cost Unclustered Solution:\n')
        f.write('Cost %.01f, Boundary %.01f\n\n' % (xCost,xBoundary))
        f.write('Minimum Boundary Clustered Solution\n')
        f.write('Cost %.01f, Boundary %.01f\n\n' % (yCost,yBoundary))
        f.write('Estimated Midpoint BLM: %.14f\n' % calcSlope)
        f.write('Cost %.01f, Boundary %.01f\n\n' % (zCost,zBoundary))
        f.write('Estimated sensitive range of BLM is between %.14f and %.14f\n' % (minRange, maxRange))
        f.write('If point Z is not clustered enough, test the Y-Z slope (line b): %.14f\n' % bVal)
        f.write('If point Z is too clustered, test the X-Z slope (line c): %.14f\n' % cVal)
        f.close()
        #
        arcpy.SetProgressorPosition(100)
    ...
    #
    #
    # stop execution if Marxan failed
    #
    def raiseIfMarxanError(self,status,messageText):
        if status == 'Error':
            arcpy.AddError(messageText)
            raise arcpy.ExecuteError
    #
    #
    # calculate slope and guard against duplicate boundary values
    #
    def calculateSlope(self,costA,costB,boundaryA,boundaryB,labelA,labelB):
        boundaryDifference = boundaryA - boundaryB
        if abs(boundaryDifference) < 0.000000000001:
            arcpy.AddError('Cannot calculate BLM slope because points %s and %s have the same boundary length' % (labelA,labelB))
            raise arcpy.ExecuteError
        return(abs((costA - costB) / boundaryDifference))
    #
    #
    # set costs to zero
    #
    ...
    def getCostBoundaryLength(self,param,scenName,outDir,inDir):
        ...
        if param == 'calcCost':
            for row in summaryData:
                rowScore = float(summaryData[row]['Score'])
                if score is None:
                    score = rowScore
        ...
        if cost is None or boundary is None or solnId is None:
            arcpy.AddError('Could not identify a best Marxan solution from %s' % csvFile)
            raise arcpy.ExecuteError
    ...
    def restoreCosts(self,inDir):
        ...
        # rename copy
        puFNCopy = os.path.join(inDir,'pu.dat.old')
        if not os.path.exists(puFNCopy):
            return
        if os.path.exists(puFName):
            os.remove(puFName)
...
class GraphBLM(object):
    ...
    def execute(self, parameters, messages):
        ...
        x = 1
        for blm in blmList:
            arcpy.SetProgressorPosition(round(90/len(blmList)*x))
...
def runMarxanOnce(exeFile,workingDir,outDir,scenName,numReps):
    ...
    def writeLog(results,fName):
        f = open(logFName+'.stdout','w', newline='')
        stuff = results[0]
    ...
    popen_kwargs = {'stdin': subprocess.PIPE, 'cwd': workingDir}
    if os.name == 'nt':
        si = subprocess.STARTUPINFO()
        si.dwFlags = subprocess.STARTF_USESTDHANDLES | subprocess.STARTF_USESHOWWINDOW
        popen_kwargs['startupinfo'] = si
    proc = subprocess.Popen(exeFile, **popen_kwargs)
    # wait
    time.sleep(5)
    ...
    if os.path.exists(logFName):
        ...
        if InputsComplete:
            ...
            while currentRun < numReps and not abort:
                ...
                currentRun = sum(1 for i in fileList if i.startswith(scenName+'_r'))
                ...
                if pStat is not None and pStat != 0:
                    abort = True
    ...
    if not abort:
        ...
        if not hasWarnings and specCount > 0:
