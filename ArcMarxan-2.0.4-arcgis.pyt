...
class Toolbox(object):
    def __init__(self):
        ...
        .pyt file)."""
        self.label = "ArcMarxan Toolbox (2.0.4 for ArcGIS)"
...
class SelectedSummary(object):
    ...
    def execute(self, parameters, messages):
        ...
        if os.path.exists(specFile):
            ...
            specRecs = {}
            with open(specFile,'rb') as csvfile:
        ...
        if os.path.exists(puvspFile):
            ...
            dialect = sniffer.sniff(contents)
            f.close()           
            with open(puvspFile,'rb') as csvfile:
...
class EstimateBLM(object):
    ...
    def getCostBoundaryLength(self,param,scenName,outDir,inDir):
        ...
        for row in summaryData:
            ...
            if bestScore is None or rowScore < bestScore:
                bestScore = rowScore
                bestRow = summaryData[row]
        if bestRow is None:
            arcpy.AddError('Marxan summary output does not contain any solution rows')
        if param == 'calcCost':
            cost = bestRow['Cost']
            boundary = bestRow['Connectivity']
        elif param == 'realCost':
            boundary = bestRow['Connectivity']
            solnId = int(bestRow['Run_Number'])
